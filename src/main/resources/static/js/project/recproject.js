    ...
    $.ajax({
        ...
        success: (r) => {
            ...
            for(let j=0; j < r[1].length; j++){
                ...
                <span>${r[1][j].employeeDto.ename}</span>
                <span>${r[1][j].employeeDto.img} </span>
                <span>${r[1][j].score}</span>
                </input> </div>`
            }
            ...
            for(let j=0; j < r[2].length; j++){
                ...
                <span>${r[2][j].employeeDto.ename}</span>
                <span>${r[2][j].employeeDto.img} </span>
                <span>${r[2][j].score}</span>
                </input> </div>`
            }
...
function s_doPost(){
    ...
    $.ajax({
        ...
        success: (r) => {
            if(r){
            alert("전송 성공")
            location.href="/sales/view?spjno="+spjno
            }
            else{
            alert("인원수 초과")
            }
...
function autowired(){
    ...
    $.ajax({
        type: 'get',
        url: '/project/view/auto?pjno='+pjno,
        success: (r) => {
                let keys = []
               ...
               let autowired = srank1.slice(0, r[0])
                               .concat(srank2.slice(0, r[1]))
                               .concat(srank3.slice(0, r[2]));
    ...
    $.ajax({
        ...
        success: (r) => {
            ...
            for(let j=0; j < r[0].length; j++){
                ...
                <td>${r[0][j].employeeDto.eno}</td>
                <td>${r[0][j].employeeDto.ename}번</td>
                <td>${r[0][j].employeeDto.img}번</td>
                <td>${r[0][j].score}</td>
                </tr>`
